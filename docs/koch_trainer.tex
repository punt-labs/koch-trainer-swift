...
Koch Trainer teaches Morse code using the \emph{Koch method}---a
proven technique
...
\begin{enumerate}
  \item \textbf{Audio-first}: Morse code is fundamentally an auditory
    skill. Visual
...
We do not model audio generation, UI state, or real-time session
mechanics---only
...
and level numbers 1--26 implicitly identify the character set
unlocked at each level.
...
In \emph{receive} mode, the student hears Morse audio and types the
corresponding
...
adapt based on performance, implementing a simplified spaced
...
over at least 20 attempts in a session, they advance to the next
level, unlocking
...
Ear training levels (1--5) correspond to pattern length: level 1
covers single-element
patterns (E, T), level 5 includes 5-element patterns like digits.
Unlike receive/send,
...
They provide motivation through gamification without affecting
learning outcomes.
...
their last practice. The longest streak updates if the current streak
exceeds it.
...
Minimum attempts scale with character count: $\max(15, 5 \times
characterCount)$.
...
The \emph{characterCount} is the number of unlocked characters for
this session's
...
A session begins in the introduction phase with the character count set to the
student's current level for the chosen direction.
...
session transitions to the training phase.
...
Pausing preserves all session state. The implementation persists paused sessions
to storage so they survive app termination.
...
When a session completes (either by proficiency or user action), the accumulated
statistics are transferred to persistent state via $RecordSession$ and the
relevant $Advance$ and $UpdateInterval$ operations.
...
\begin{enumerate}
  \item On session completion, calculate accuracy: $sessionCorrect
    \div sessionAttempts$
  \item If accuracy $\geq 90\%$: invoke $AdvanceReceiveLevel$ or
    $AdvanceSendLevel$
  ...
  \item Update streak via $IncrementStreak$ or $ResetStreak$ based on
    calendar day
\end{enumerate}
...
This specification focuses on state that affects learning outcomes.
The following
