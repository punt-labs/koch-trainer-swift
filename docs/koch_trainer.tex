...
\begin{itemize}
\item \emph{radioOff}: Radio not active (session not started, paused, or completed)
\item \emph{radioReceiving}: Radio on, listening---continuous noise floor with
      band conditions (QRN, QSB, QRM), incoming Morse signals audible
\item \emph{radioTransmitting}: Radio on, sending---sidetone only, receiver muted
...
accuracy? \geq advanceThreshold \\
accuracy? \leq 100 \\
\LET effectiveMax == \IF daysSinceStart? < habitFormationDays
    \THEN habitMaxInterval \ELSE maxInterval @
(receiveInterval * 2 \leq effectiveMax \land
    receiveInterval' = receiveInterval * 2) \lor
(receiveInterval * 2 > effectiveMax \land
    receiveInterval' = effectiveMax) \\
sendInterval' = sendInterval \\
currentStreak' = currentStreak \\
longestStreak' = longestStreak \\
receiveLevel' = receiveLevel \\
...
\LET effectiveMax == \IF daysSinceStart? < habitFormationDays
    \THEN habitMaxInterval \ELSE maxInterval @
(sendInterval * 2 \leq effectiveMax \land
    sendInterval' = sendInterval * 2) \lor
(sendInterval * 2 > effectiveMax \land
    sendInterval' = effectiveMax) \\
receiveInterval' = receiveInterval \\
currentStreak' = currentStreak \\
longestStreak' = longestStreak \\
receiveLevel' = receiveLevel \\
...
This section models the state machine for training sessions. While persistent
state (levels, streaks, intervals) survives across sessions, session state
exists only during active training.
...
\begin{zed}
SessionPhase ::= introduction | training | paused | completed
\end{zed}
...
\item \emph{completed}: Session ended, results recorded
\end{itemize}
...
Session state is separate from persistent progress state. In the implementation,
this lives in the ViewModel; here we model it as a distinct schema.

\begin{schema}{SessionState}
phase : SessionPhase \\
direction : Direction \\
radioMode : RadioMode \\
toneActive : ZBOOL \\
...
phase \in \{introduction, paused, completed\} \implies radioMode = radioOff \\
(phase = training \land direction = receive) \implies radioMode = radioReceiving \\
radioMode = radioTransmitting \implies direction = send \\
% Tone constraints
radioMode = radioOff \implies toneActive = zfalse
\end{schema}
...
A session begins in the introduction phase with the radio off. The radio
activates when training begins (transitioning to receiving mode).
...
\begin{itemize}
\item \emph{radioReceiving}: Tone mixed with noise floor and band conditions (QRN, QSB, QRM)
\item \emph{radioTransmitting}: Clean sidetone only (no noise or band conditions)
\item \emph{radioOff}: No audio output (toneActive must be false)
\end{itemize}
...
$ActivateTone$ requires the radio to be on (either receiving or transmitting).
...
\where
phase = training \\
\LET minAttempts == \IF baseMinimumAttempts > attemptsPerCharacter * characterCount
    \THEN baseMinimumAttempts
    \ELSE attemptsPerCharacter * characterCount @
\LET accuracyPercent == (sessionCorrect * 100) \div sessionAttempts @
(sessionAttempts \geq minAttempts \land accuracyPercent \geq proficiencyThreshold
    \land proficiencyMet! = ztrue) \lor
(sessionAttempts < minAttempts \land proficiencyMet! = zfalse) \lor
(accuracyPercent < proficiencyThreshold \land proficiencyMet! = zfalse)
\end{schema}
