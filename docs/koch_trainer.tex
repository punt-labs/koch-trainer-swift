...
The complete machine state combines persistent progress with session state.
This gives ProB a single state schema to track during animation.

\begin{zed}
MachineState \defs State \land SessionState
\end{zed}
...
currentStreak' = 0 \\
longestStreak' = 0 \\
% Session state initialization (idle - no active session)
phase' = completed \\
direction' = receive \\
radioMode' = off \\
toneActive' = zfalse \\
